...
import type {Playlist, Video, PlayPauseAction, NextPreviousAction, AddVideoAction, DeleteVideoAction, UpdatePlaylistAction} from "./types";
...
export const play = (firebase: Firebase): PlayPauseAction=> {
  return (id: string) => (
    firebase.update(positionPath(id), {state: constants.PLAYING, switchingSongs: false})
  )
}
...
export const next = (firebase: Firebase): NextPreviousAction => {
  return (id: string, playlist: Playlist, isPlaying?: boolean) => {
    ...
    const changes: {switchingSongs?: boolean} = { video: playlist.order[newPosition]}
    if (isPlaying) {
...
export const deleteVideo =(firebase: Firebase): DeleteVideoAction => {
  return async (playlistId: string, playlist: Playlist, id: string) => {
    ...
    return firebase.ref().child(`${videosPath(playlistId)}/${id}`).remove()
  }
}
